...
def reconcile_discovered_tasks(state, candidates):
    """Keep Drive discovery as an ephemeral candidate pool, not canonical lifecycle state."""
    result = copy.deepcopy(state)
    result["discovered_candidates"] = copy.deepcopy(candidates or [])
    result.setdefault("authority_gate", {})["last_action"] = "WORKSPACE_DISCOVERY_CANDIDATES"
    return result
